...
```{R}
Batch4_ATAC <- RenameIdents(Batch4_ATAC, `Glu_Neurons` = "Neurons",`GABA Neurons` = "Neurons", `OPCs` = "Oligodendrocytes", `Glu_GABA neurons` = "Neurons",`Dopaminergic Neurons group 1` = "Neurons")
```
...
DimPlot(Batch4_ATAC, group.by = "grouped_id", label = TRUE, repel = TRUE)
...
write.table(ATAC_bed,file="Files/ATAC_peaks_all.bed",sep = "\t", col.names = FALSE, row.names = FALSE)
...
```{R}
Astrocyte_Peaks <- read.delim("~/Documents/Projects/PD5D_Repository/scRNA_and_scATAC-seq_Markdowns/batch4/scATAC-seq/Files/macs2/Astrocytes_peaks.narrowPeak", stringsAsFactors = FALSE, sep = "\t")

Astrocyte_Peaks_R1 <- gsub("X","",colnames(Astrocyte_Peaks))
Astrocyte_Peaks <- rbind(Astrocyte_Peaks_R1, Astrocyte_Peaks)
