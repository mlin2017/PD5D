...
```{r}
Batch567_MTG <- RunPCA(Batch567_MTG, features = VariableFeatures(object = Batch567_MTG), npcs=30)
```
...
Run Harmony
```{r}
Batch567_MTG <- RunHarmony(Batch567_MTG, group.by.vars = c("batch","case","sample_ID","sex","age_bracket"), reduction = "pca", plot_convergence = TRUE, theta = c(0.4,0.4,0.4,0.4,0.4))
```
...
ElbowPlot(Batch567_MTG, reduction = "pca")
...
```{r}
Batch567_MTG <- FindNeighbors(Batch567_MTG, reduction = "harmony", dims = 1:18)
...
```{r}
Batch567_MTG <- RunUMAP(Batch567_MTG, reduction = "harmony", dims = 1:18)
```
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_Leiden_FullCorrectionHarmony_PCA_UMAPclusters_scRNA_seq_Batch123_MTG.pdf", device = "pdf", width = 6, height = 4, units = "in")
```
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_Leiden_FullCorrectionHarmony_CaseSplit_PCA_UMAPclusters_scRNA_seq_Batch123_MTG.pdf", device = "pdf", width = 6, height = 10, units = "in")
```
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_Leiden_FullCorrectionHarmony_CaseGroup_PCA_UMAPclusters_scRNA_seq_Batch123_MTG.pdf", device = "pdf", width = 6, height = 4, units = "in")
```
