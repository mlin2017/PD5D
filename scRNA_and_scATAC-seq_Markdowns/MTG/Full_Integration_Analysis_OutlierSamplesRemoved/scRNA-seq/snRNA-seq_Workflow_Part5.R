...
ExpVectorPerCluster <- lapply(sort(unique(as.numeric(as.vector(SeuratObject@active.ident)))),FUN = ClusterExpMed, SeuObj = SeuratObject)
...
write.table(as.data.frame(CellClusterAssignment), file = "Files/PanglaoCellClusterAssignment.tsv", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
