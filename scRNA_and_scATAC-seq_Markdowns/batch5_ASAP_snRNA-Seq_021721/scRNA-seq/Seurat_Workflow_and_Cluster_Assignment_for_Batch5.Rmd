...
```{r}
Batch5_MTG <- subset(Batch5_MTG, subset = nFeature_RNA > 200 & percent.mt < 5)
```
...
```{r}
Batch5_MTG <- RunPCA(Batch5_MTG, npcs = 30, verbose = FALSE)
```
...
Run Harmony
```{r}
Batch5_MTG <- RunHarmony(Batch5_MTG, group.by.vars = "case", plot_convergence = TRUE)
```
...
```{r}
Batch5_MTG <- FindNeighbors(Batch5_MTG, reduction = "harmony", dims = 1:20)
Batch5_MTG <- FindClusters(Batch5_MTG, resolution = 0.5)
```

run Umap based on top 20 harmony axis: 

```{r}
Batch5_MTG <- RunUMAP(Batch5_MTG, reduction = "harmony", dims = 1:20)
```

run tsne based on top 20 harmony axis: 

```{r}
Batch5_MTG <- RunTSNE(Batch5_MTG, reduction = "harmony", dims = 1:20)
```
...
```{r}
DimPlot(Batch5_MTG, reduction = "umap", group.by = "experiment",pt.size = 0.1)
```

TSNE Plot cells colored by clusters and grouped by case. 
```{r}
DimPlot(Batch5_MTG, reduction = "tsne", split.by = "case", label = TRUE, ncol = 1)
```
