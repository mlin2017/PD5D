...
cellcodes <- as.data.frame(Batch8.data@Dimnames[[2]])
...
cellcodes$samples <- as.vector(Batch8samples$library_id[cellcodes$libcodes])
...
```{R}
Batch8samples$case <- c("HC","PD","ILBD","ILBD")

cellcodes$case <- as.vector(Batch8samples$case[cellcodes$libcodes])
...
cellcodes$batch <- "Batch_8"

cellcodes$region <- "MTG"

batch8metadata <- cellcodes[,3:6]
...
cellcodes <- as.data.frame(Batch7.data@Dimnames[[2]])
colnames(cellcodes) <- "barcodes"
rownames(cellcodes) <- cellcodes$barcodes

cellcodes$libcodes <- as.factor(gsub(pattern=".+-", replacement="", cellcodes$barcodes))
cellcodes$samples <- as.vector(batch7samples$library_id[cellcodes$libcodes])

```


```{R}
batch7samples$case <- c("HC","PD","ILBD","ILBD")

cellcodes$case <- as.vector(batch7samples$case[cellcodes$libcodes])
...
cellcodes$batch <- "Batch_7"

cellcodes$region <- "MTG"

batch7metadata <- cellcodes[,3:6]
...
```{R}
MB_sample <- c(rep("BRI318", ncol(BRI318.data)),
...
```{R}
MB_case <- c(rep("PD", ncol(BRI318.data)),
...
```{R}
MB_batch <- c(rep("Pilot_1", ncol(BRI318.data)),
                          rep("Pilot_1", ncol(BRI319.data)),
                          rep("Pilot_1", ncol(BRI320.data)),
                          rep("Pilot_1", ncol(BRI321.data)),
                          rep("Pilot_2", ncol(B0085.data)),
                          rep("Pilot_2", ncol(H0321.data)),
                          rep("Pilot_2", ncol(H1009.data)),
                          rep("Pilot_2", ncol(H1118.data)))
```


```{R}
MB_metadata <- data.frame(cbind(MB_sample,MB_case,MB_batch))

MB_metadata$region <- "Midbrain"

colnames(MB_metadata) <- colnames(batch7metadata)

integration_metadata <- rbind(MB_metadata,batch7metadata,batch8metadata)
...
genes2<-rownames(Batch7.data)
genes_common<-intersect(genes1,genes2)
genes_notcommon<- genes1[!genes1 %in% genes2]
genes_notcommon2 <- genes2[!genes2 %in% genes1]
gene_nc <- unique(c(genes_notcommon,genes_notcommon2))
...
MTGandMidbrainIntegration <- CreateSeuratObject(counts = cbind(BRI318.data,
                                           BRI319.data,
                                           BRI320.data,
                                           BRI321.data,
                                           B0085.data,
