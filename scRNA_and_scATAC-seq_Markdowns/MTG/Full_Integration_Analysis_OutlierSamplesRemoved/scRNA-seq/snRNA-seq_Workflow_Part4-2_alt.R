...
dir.create("Files/Subclusters_GSEA")
dir.create("Files/Subclusters_DE_Genes")
...
ClusterfGSEA <- function(ClusterIdent, IdentObj, ident1, ident2){
  ...
  write.table(fgseaResAll, file = paste("Files/Subclusters_GSEA/",ClusterIdent,"_",ident1,"_vs_",ident2,"_GSEA_AllGenesets.tsv",sep = ""), quote = FALSE, col.names = TRUE, row.names = FALSE, sep = "\t")
  ...
  write.table(fgseaResFilterBH, file = paste("Files/Subclusters_GSEA/",ClusterIdent,"_",ident1,"_vs_",ident2,"_GSEA_SigBHCorrection_Genesets.tsv",sep = ""), quote = FALSE, col.names = TRUE, row.names = FALSE, sep = "\t")
  }
  fgseaResFilter <- fgseaRes[fgseaRes$pval <= fgseaRes$BF_pval,]
  if (nrow(fgseaResFilter) > 0) {
  write.table(fgseaResFilter, file = paste("Files/Subclusters_GSEA/",ClusterIdent,"_",ident1,"_vs_",ident2,"_GSEA_SigBFCorrection_Genesets.tsv",sep = ""), quote = FALSE, col.names = TRUE, row.names = FALSE, sep = "\t")
  }
  ...
  write.csv(IdentObj.Markers, file = paste("Files/Subclusters_DE_Genes/AllGenes_",ClusterIdent,"_Markers_",ident1,"_vs_",ident2,".csv",sep = ""), quote = FALSE)
}
...
ClusterDE <- function(ClusterIdent, IdentObj, ident1, ident2){
  ...
  if (length(Ident.PDvsHC_Prog.Markers$gene) != 0) {
        ...
        if (length(Ident.PDvsHC_Prog.MarkersBF$gene) != 0) {
            write.csv(Ident.PDvsHC_Prog.MarkersBF,file = paste("Files/Subclusters_DE_Genes/All_SigGenes_",Cluster_Ident,"_Markers_",ident1,"_vs_",ident2,"_BF.csv",sep = ""), quote = FALSE)
        }
        ...
        if (length(Ident.PDvsHC_Prog.MarkersBH$gene) != 0) {
            write.csv(Ident.PDvsHC_Prog.MarkersBH,file = paste("Files/Subclusters_DE_Genes/All_SigGenes_",Cluster_Ident,"_Markers_",ident1,"_vs_",ident2,"_BH.csv",sep = ""), quote = FALSE)
        }
}
}
