...
ggsave(UMAPclusters, filename = "Figures/Unassigned_GLMAPCA_Leiden_UMAPclusters_scRNA_seq_Batch7_MTG_2.pdf", device = "pdf", width = 6, height = 6, units = "in")
```
...
Batch7_MTG_counts_matrix <- Batch7_MTG@assays$RNA@counts

saveRDS(Batch7_MTG_counts_matrix, "Files/GLMPCA_Batch7_MTG_Unassigned_counts_matrix.rds")
...
DetRate <- as.vector(scale(colSums(Batch7_MTG@assays$RNA@counts > 0)))
...
```{r}
VlnPlot(Batch7_MTG, features = c("SH3GL2"), pt.size = 0)
```


```{r}
VlnPlot(Batch7_MTG, features = c("CD4"),pt.size = 0)
```
...
```{r}
VlnPlot(Batch7_MTG, features = c("NEUROD1"),pt.size = 1)
```


```{r}
VlnPlot(Batch7_MTG, features = c("DCX"),pt.size = 1)
```


```{r}
VlnPlot(Batch7_MTG, features = c("SOX2"),pt.size = 1)
```
```{r}
VlnPlot(Batch7_MTG, features = c("NES"),pt.size = 1)
```
```{r}
VlnPlot(Batch7_MTG, features = c("TUBB3"),pt.size = 1)
```
