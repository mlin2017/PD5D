...
metadata$region <- "Midbrain"

metadata$region[metadata$batch %in% c("Batch7","Batch8")] <- "MTG"
...
BatchMB_MTG <- CreateSeuratObject(counts = do.call(cbind,lapply(Batches,get)),
                            project = "BatchMB_MTG",
                            min.cells = 3)
```
...
BatchMB_MTG@meta.data$sample_id <- metadata$samples
BatchMB_MTG@meta.data$case <- metadata$case
BatchMB_MTG@meta.data$batch <- metadata$batch
...
```{r}
table(BatchMB_MTG$sample_id)
```

```{R}
table(BatchMB_MTG$case)
```
...
library(stats)
nfeature_RNA <- BatchMB_MTG@meta.data$nFeature_RNA
mean(nfeature_RNA)
MAD <- mad(nfeature_RNA, center = median(nfeature_RNA))
threeMAD <- (MAD*3)+median(BatchMB_MTG@meta.data$nFeature_RNA)
...
```{r}
BatchMB_MTG <- FindNeighbors(BatchMB_MTG, reduction = "harmony", dims = 1:13)
...
```{r}
BatchMB_MTG <- RunUMAP(BatchMB_MTG, reduction = "harmony", dims = 1:13)
```
