...
```{r}
Batch8_MTG <- RunGLMPCA(Batch8_MTG, features=Batch8_MTG@assays$RNA@var.features, L = 30)
```

see contribution of genes to construct each of these principal components.
```{r}
VizDimLoadings(Batch8_MTG, dims = 1:2, reduction = "glmpca")
```


plot cells in the first two principal components colored by case: 
```{r}
DimPlot(object = Batch8_MTG, reduction = "glmpca", pt.size = .1, group.by = "case")
```
...
Run Harmony
```{r}
Batch8_MTG <- RunHarmony(Batch8_MTG, group.by.vars = c("case","sample_ID"), reduction = "glmpca", plot_convergence = TRUE, theta = c(1,1))
```
...
ElbowPlot(Batch8_MTG, reduction = "glmpca")
...
ElbowPlot(Batch8_MTG, reduction = "harmony")
...
```{r}
Batch8_MTG <- FindNeighbors(Batch8_MTG, reduction = "harmony", dims = 1:16)
Batch8_MTG <- FindClusters(Batch8_MTG, resolution = 0.5, algorithm = 4)
```

run Umap based on top 10 harmony axis: 

```{r}
Batch8_MTG <- RunUMAP(Batch8_MTG, reduction = "harmony", dims = 1:16)
```
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_GLMAPCA_UMAPclusters_scRNA_seq_Batch8_MTG.pdf", device = "pdf", width = 6, height = 4, units = "in")
```


```{R}
UMAPclusters <- DimPlot(Batch8_MTG, label = TRUE, repel = TRUE, pt.size = 0, label.size = 2.5, split.by = "case") + 
        theme(axis.text = element_text(size=8),
              axis.title = element_text(size = 12),
              legend.text = element_text(size = 8),
              title = element_text(size = 12),
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_Split_GLMPCA_UMAPclusters_scRNA_seq_Batch8_MTG.pdf", device = "pdf", width = 12, height = 4, units = "in")
```
