---
title: "Seurat Workflow and Cluster Assignment for Pilot_1"
...
```{r}
Pilot_1_MB <- CreateSeuratObject(counts = cbind(Pilot_1.data),
                            project = "Pilot_1_MB",
                            min.cells = 3)
```
...
```{R}
samples <- read.csv(file.path("~/Documents/Projects/PD5D_Repository/scRNA_and_scATAC-seq_Markdowns/Pilot_1/scRNA-seq/Files/cellranger_matrices/cellranger_aggr_matrices", "aggregation.csv"), stringsAsFactors=F)

cellcodes <- as.data.frame(Pilot_1_MB@assays$RNA@data@Dimnames[[2]])
...
Pilot_1_MB@meta.data$sample_ID <- cellcodes$samples
...
Pilot_1_MB@meta.data$case <- cellcodes$case

Pilot_1_MB@meta.data$region <- "Midbrain"
...
```{r}
Pilot_1_MB[["percent.mt"]] <- PercentageFeatureSet(Pilot_1_MB, pattern = "^MT-")
VlnPlot(Pilot_1_MB, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
```
...
```{R}
library(stats)
nfeature_RNA <- Pilot_1_MB@meta.data$nFeature_RNA
mean(nfeature_RNA)
MAD <- mad(nfeature_RNA, center = median(nfeature_RNA))
threeMAD <- (MAD*3)+median(Pilot_1_MB@meta.data$nFeature_RNA)

```

```{R}

testing <- subset(Pilot_1_MB, subset = case == "HC")
```
...
```{r}
Pilot_1_MB <- subset(Pilot_1_MB, subset = nFeature_RNA > 200 & nFeature_RNA < threeMAD | percent.mt < 5)
```
...
```{r}
Pilot_1_MB <- NormalizeData(Pilot_1_MB, normalization.method = "LogNormalize", scale.factor = 10000)
```
...
```{R}
Pilot_1_MB <- FindVariableFeatures(Pilot_1_MB, selection.method = "vst", nfeatures = 2000)

# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(Pilot_1_MB), 10)

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(Pilot_1_MB)
...
```{r}
all.genes <- rownames(Pilot_1_MB)
```

center and scale data
```{r}
Pilot_1_MB <- ScaleData(Pilot_1_MB, features = all.genes, verbose = FALSE)
```

finding the top 30 principal components for cells
```{r}
Pilot_1_MB <- RunGLMPCA(Pilot_1_MB, features=Pilot_1_MB@assays$RNA@var.features, L = 30)
```

see contribution of genes to construct each of these principal components.
```{r}
VizDimLoadings(Pilot_1_MB, dims = 1:2, reduction = "glmpca")
```


plot cells in the first two principal components colored by case: 
```{r}
DimPlot(object = Pilot_1_MB, reduction = "glmpca", pt.size = .1, group.by = "case")
```

Coordinate of cells in PC 1 characterized by case: 
```{r}
VlnPlot(object = Pilot_1_MB, features = "PC_1", group.by = "case",  pt.size = .1)
```

Run Harmony
```{r}
Pilot_1_MB <- RunHarmony(Pilot_1_MB, group.by.vars = c("sample_ID","case"), plot_convergence = TRUE, reduction = "glmpca", theta=c(1,1))
```
...
ElbowPlot(Pilot_1_MB, reduction = "harmony")
...
```{r}
Pilot_1_MB <- FindNeighbors(Pilot_1_MB, reduction = "harmony", dims = 1:14)
Pilot_1_MB <- FindClusters(Pilot_1_MB, resolution = 0.5, algorithm = 4)
```

run Umap based on top 20 harmony axis: 

```{r}
Pilot_1_MB <- RunUMAP(Pilot_1_MB, reduction = "harmony", dims = 1:14)
```
...
plot umap: 
```{r}
DimPlot(Pilot_1_MB, reduction = "umap", label = TRUE,pt.size = 0.01)
```

```{r}
DimPlot(Pilot_1_MB, reduction = "umap", group.by = "case",pt.size = 0.1)
```


TSNE Plot cells colored by clusters and grouped by case. 
```{r}
DimPlot(Pilot_1_MB, reduction = "umap", split.by = "case", label = TRUE, ncol = 1)
```

Now let's plot cells with the assigned celltypes: 
```{r}
DimPlot(Pilot_1_MB, label = TRUE, repel = TRUE, pt.size = 0, label.size = 2)

UMAPclusters <- DimPlot(Pilot_1_MB, label = TRUE, repel = TRUE, pt.size = 0, label.size = 2.5) + 
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_UMAPclusters_scRNA_seq_Pilot_1.pdf", device = "pdf", width = 6, height = 4, units = "in")
```


```{R}
UMAPclusters <- DimPlot(Pilot_1_MB, label = TRUE, repel = TRUE, pt.size = 0, label.size = 2.5, split.by = "case") + 
        theme(axis.text = element_text(size=8),
              axis.title = element_text(size = 12),
              legend.text = element_text(size = 8),
              title = element_text(size = 12),
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_Split_UMAPclusters_scRNA_seq_Pilot_1.pdf", device = "pdf", width = 12, height = 4, units = "in")
```
...
```{r}
saveRDS(Pilot_1_MB,"Files/Pilot_1_MB_Unassigned.rds")
```
...
```{R}
Pilot_1_MB=readRDS("Files/Pilot_1_MB_Unassigned.rds")
```
...
```{r}
VlnPlot(Pilot_1_MB, features = c("CD96"),pt.size = 0)
```
NK Cell Markers

```{r}
VlnPlot(Pilot_1_MB, features = c("IL7R"),pt.size = 0)
```
...
```{r}
VlnPlot(Pilot_1_MB, features = c("FCGR3A"),pt.size = 0)
```

CD56

```{r}
VlnPlot(Pilot_1_MB, features = c("NCAM1"),pt.size = 0)
```
...
```{r}
VlnPlot(Pilot_1_MB, features = c("KLRD1"),pt.size = 0)
```
NKG2A

```{r}
VlnPlot(Pilot_1_MB, features = c("KLRC1"),pt.size = 0)
```
```{r}
VlnPlot(Pilot_1_MB, features = c("CD244"),pt.size = 0)
```

resting T cell marker?

```{r}
VlnPlot(Pilot_1_MB, features = c("CD226"),pt.size = 0)
```

```{r}
VlnPlot(Pilot_1_MB, features = c("CD8A"),pt.size = 0)
```
```{r}
...
```{r}
TKillerCellMarkerVlnPlot <- VlnPlot(Pilot_1_MB, features = c("CD8A","CD226","CD244","CD96","IL7R","KLRD1","NCAM1","FCGR3A","KLRC1"),pt.size = 0, ncol = 1) 
...
```{r}
VlnPlot(Pilot_1_MB, features = c("ENO2"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("RBFOX3"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("SLC17A6"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("SLC17A7"),pt.size = 0)
```

GLU 9,10,19

```{R}
VlnPlot(Pilot_1_MB, features = c("SLC32A1"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("GAD1"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("GAD2"),pt.size = 0)
```
GABA - 5,6,10,16,18


```{R}
VlnPlot(Pilot_1_MB, features = c("TH"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("SLC6A3"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("SLC18A2"),pt.size = 0)
```
...
```{R}
VlnPlot(Pilot_1_MB, features = c("AQP4"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("GFAP"),pt.size = 0)
```


```{R}
VlnPlot(Pilot_1_MB, features = c("ALDH1L1"),pt.size = 0)
```
...
```{R}
VlnPlot(Pilot_1_MB, features = c("PLP1"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("MBP"),pt.size = 0)
```
...
```{R}
VlnPlot(Pilot_1_MB, features = c("VCAN"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("BCAN"),pt.size = 0)
```
OPCs - 6


```{R}
VlnPlot(Pilot_1_MB, features = c("CX3CR1"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("P2RY12"),pt.size = 0)
```
...
```{R}
VlnPlot(Pilot_1_MB, features = c("FLT1"),pt.size = 0)
```
```{R}
VlnPlot(Pilot_1_MB, features = c("CLDN5"),pt.size = 0)
```
...
```{R}
VlnPlot(Pilot_1_MB, features = c("RELN"),pt.size = 0)
```


```{R}
VlnPlot(Pilot_1_MB, features = c("CALB2"),pt.size = 0)
```


```{R}
VlnPlot(Pilot_1_MB, features = c("CNR1"),pt.size = 1)
```

```{R}
VlnPlot(Pilot_1_MB, features = c("PAFAH1B1"),pt.size = 1)
```
...
```{r}
Pilot_1_MB <- RenameIdents(Pilot_1_MB, `1` = "Oligodendrocytes", `2` = "Astrocytes",
...
ggsave(UMAPclusters, filename = "Figures/UMAPclusters_scRNA_seq_Pilot_1.pdf", device = "pdf", width = 6, height = 4, units = "in")
```
...
ggsave(UMAPclusters, filename = "Figures_Pilot_1/Split_UMAPclusters_scRNA_seq_Pilot_1.pdf", device = "pdf", width = 12, height = 4, units = "in")
```
