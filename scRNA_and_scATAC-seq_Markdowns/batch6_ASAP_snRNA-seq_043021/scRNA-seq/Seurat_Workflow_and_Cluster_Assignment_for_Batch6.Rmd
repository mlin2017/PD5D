...
```{r}
Batch5.data <- Read10X(data.dir = "Files/cellranger_matrices/cellranger_aggr_matrices/filtered_feature_bc_matrix")
```
...
```{R}
samples$case <- c("HC","HC","ILBD","ILBD")
...
```{r}
Batch5_MTG <- FindNeighbors(Batch5_MTG, reduction = "harmony", dims = 1:17)
...
```{r}
Batch5_MTG <- RunUMAP(Batch5_MTG, reduction = "harmony", dims = 1:17)
```
...
```{r}
DimPlot(Batch5_MTG, reduction = "umap", split.by = "case", label = TRUE, ncol = 1)
```

```{r}
Batch5_MTG_Case_UMAP <- DimPlot(Batch5_MTG, reduction = "umap", split.by = "case", label = TRUE, ncol = 1) + 
...
ggsave(Batch5_MTG_Case_UMAP, filename = "Figures/Batch5_MTG_Case_UMAP.pdf", device = "pdf", width = 6, height = 8, units = "in")
```
