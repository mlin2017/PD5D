...
VarGenes <- read.delim("Files/VarGenes.tsv")
...
for (batch_id in Batches) {
    #object_name <- paste(as.character(as.name(batch_id)),"_object",sep = "")
    assign(as.character(as.name(batch_id)), CreateSeuratObject(counts = get(batch_id),
              project = gsub(".data","",as.name(batch_id)),
              min.cells = 3))
    assign(as.character(as.name(batch_id)),subset(get(batch_id), features = VarGenes))
}

SeuratObject <- merge(get(Batches[1]), 
                 y = lapply(Batches[2:length(Batches)],get), 
                 add.cell.ids = Batches, 
                 project = "FullIntegration_MTG")

write.table(as.data.frame(Batches),file = "Files/Batch_Orders.tsv", sep = "\t", quote = FALSE)
#####################alt1############################
...
#for (batch_id in Batches) {
#tempobj <- CreateSeuratObject(counts = get(batch_id),
#              project = gsub(".data","",as.name(batch_id)),
#              min.features = 200)
#assign(as.character(as.name(batch_id)), tempobj@assays$RNA@counts)
#}
...
#SeuratObject <- Read10X(data.dir = "/n/scratch3/users/j/jap0606/FullIntegration/PreCombinedFiles/")

#SeuratObject <- CreateSeuratObject(counts = SeuratObject,
#                            project = "FullIntegration_MTG",
#                            min.cells = 100,
#                            min.features = 200)
...
MADdataframe <- as.data.frame(MAD)
write.table(MADdataframe, file = "Files/MADthreeframe.tsv", sep = "\t")

SeuratObject <- subset(SeuratObject, subset = nFeature_RNA > 200 & nfeature_RNA < threeMAD & percent.mt < 5)

saveRDS(SeuratObject,"/n/scratch3/users/j/jap0606/FullIntegration/FullIntegration_CombinedFilteredMatrix_test.rds")
