---
title: "Seurat Workflow and Cluster Assignment for Batch 8"
...
```{r}
Batch8.data <- Read10X(data.dir = "Files/cellranger_matrices/cellranger_aggr_matrices/filtered_feature_bc_matrix")
```
...
```{r}
Batch8_MTG <- CreateSeuratObject(counts = cbind(Batch8.data),
                            project = "Batch8_MTG",
                            min.cells = 3)
```
...
cellcodes <- as.data.frame(Batch8_MTG@assays$RNA@data@Dimnames[[2]])
...
Batch8_MTG@meta.data$sample_ID <- cellcodes$samples
...
Batch8_MTG@meta.data$case <- cellcodes$case
...
```{r}
table(Batch8_MTG$sample_ID)
```

```{R}
table(Batch8_MTG$case)
```
...
```{r}
Batch8_MTG[["percent.mt"]] <- PercentageFeatureSet(Batch8_MTG, pattern = "^MT-")

VlnPlot(Batch8_MTG, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
```
...
```{R}
library(stats)
nfeature_RNA <- Batch8_MTG@meta.data$nFeature_RNA
mean(nfeature_RNA)
MAD <- mad(nfeature_RNA, center = median(nfeature_RNA))
threeMAD <- (MAD*3)+median(Batch8_MTG@meta.data$nFeature_RNA)
...
```{r}
Batch8_MTG <- subset(Batch8_MTG, subset = nFeature_RNA > 200 & nFeature_RNA > threeMAD | percent.mt < 5)
```
...
```{r}
Batch8_MTG <- NormalizeData(Batch8_MTG, normalization.method = "LogNormalize", scale.factor = 10000)
```
...
```{R}
Batch8_MTG <- FindVariableFeatures(Batch8_MTG, selection.method = "vst", nfeatures = 2000)

# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(Batch8_MTG), 10)

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(Batch8_MTG)
...
```{r}
all.genes <- rownames(Batch8_MTG)
```

center and scale data
```{r}
Batch8_MTG <- ScaleData(Batch8_MTG, features = all.genes, verbose = FALSE)
```
...
```{r}
Batch8_MTG <- RunPCA(Batch8_MTG, npcs = 30, verbose = FALSE)
```

see contribution of genes to construct each of these principal components.
```{r}
VizDimLoadings(Batch8_MTG, dims = 1:2, reduction = "pca")
```


plot cells in the first two principal components colored by case: 
```{r}
DimPlot(object = Batch8_MTG, reduction = "pca", pt.size = .1, group.by = "case")
```

Coordinate of cells in PC 1 characterized by case: 
```{r}
VlnPlot(object = Batch8_MTG, features = "PC_1", group.by = "case",  pt.size = .1)
```

Run Harmony
```{r}
Batch8_MTG <- RunHarmony(Batch8_MTG, group.by.vars = "case", plot_convergence = TRUE)
```


cells in harmony axis
```{r}
harmony_embeddings <- Embeddings(Batch8_MTG, 'harmony')
harmony_embeddings[1:5, 1:5]
```

cells in harmony axis
```{r}
DimPlot(object = Batch8_MTG, reduction = "harmony", pt.size = .1, group.by = "case")
```
cells in harmony 1 axis
```{r}
VlnPlot(object = Batch8_MTG, features = "harmony_1", group.by = "case",  pt.size = .1)
```
...
```{R}
#Batch8_MTG <- JackStraw(Batch8_MTG, num.replicate = 100)
#Batch8_MTG <- ScoreJackStraw(Batch8_MTG, dims = 1:12)
#JackStrawPlot(Batch8_MTG, dims = 1:20)
...
```{R}
ElbowPlot(Batch8_MTG)

Batch8_MTG_ElbowPlot <- ElbowPlot(Batch8_MTG)

ggsave2("Figures/Batch8_MTG_ElbowPlot.pdf", Batch8_MTG_ElbowPlot, device = "pdf", width = 4, height = 4, units = "in")
```
...
```{r}
Batch8_MTG <- FindNeighbors(Batch8_MTG, reduction = "harmony", dims = 1:11)
Batch8_MTG <- FindClusters(Batch8_MTG, resolution = 0.5)
```

run Umap based on top 10 harmony axis: 

```{r}
Batch8_MTG <- RunUMAP(Batch8_MTG, reduction = "harmony", dims = 1:11)
```

run tsne based on top 10 harmony axis: 

```{r}
#Batch8_MTG <- RunTSNE(Batch8_MTG, reduction = "harmony", dims = 1:17)
```

plot umap: 
```{r}
DimPlot(Batch8_MTG, reduction = "umap", label = TRUE,pt.size = 0.01)
```

```{r}
DimPlot(Batch8_MTG, reduction = "umap", group.by = "case",pt.size = 0.1)
```
TSNE Plot cells colored by clusters and grouped by case. 
```{r}
DimPlot(Batch8_MTG, reduction = "umap", split.by = "case", label = TRUE, ncol = 1)
```

```{r}
Batch8_MTG_Case_UMAP <- DimPlot(Batch8_MTG, reduction = "umap", split.by = "case", label = TRUE, ncol = 1) + 
...
ggsave(Batch8_MTG_Case_UMAP, filename = "Figures/Batch8_MTG_Case_UMAP.pdf", device = "pdf", width = 6, height = 8, units = "in")
```


TSNE Plot cells colored by clusters and grouped by case. 
```{r}
#Batch8_MTG_Case_tsne <- DimPlot(Batch8_MTG, reduction = "tsne", split.by = "case", label = TRUE, ncol = 1) + 
#        theme(axis.text = element_text(size=8),
#              axis.title = element_text(size = 12),
#              legend.text = element_text(size = 8),
#              title = element_text(size = 12),
...
#ggsave(Batch8_MTG_Case_tsne, filename = "Figures/Batch8_MTG_Case_tsne.pdf", device = "pdf", width = 6, height = 8, units = "in")
```

Find markers for every cluster compared to all remaining cells, report only the positive ones
```{r}
Batch8_MTG.markers <- FindAllMarkers(Batch8_MTG, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
```
...
```{r}
unique(Batch8_MTG.markers$cluster)
```

Top marker genes for each clusters: 
```{r}
Batch8_MTG.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
write.table(Batch8_MTG.markers, file = "Files/AllMarkers.txt", col.names = TRUE, sep = "\t", quote = FALSE)
top10Markers <- Batch8_MTG.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
...
DoHeatmap(Batch8_MTG, features = features, size = 2, draw.lines = FALSE, angle = 45,
...
```{r}
FeaturePlot(Batch8_MTG, features = c("ENO2", "RBFOX3","SLC17A6", "SLC17A7","AQP4", "GFAP"))
```

```{r}
saveRDS(Batch8_MTG,"Files/Batch8_MTG_Unassigned.rds")
```
...
```{R}
Batch8_MTG=readRDS("Files/Batch8_MTG_Unassigned.rds")
```
...
```{r}
VlnPlot(Batch8_MTG, features = c("CD96"),pt.size = 0)
```
NK Cell Markers

```{r}
VlnPlot(Batch8_MTG, features = c("IL7R"),pt.size = 0)
```
...
```{r}
VlnPlot(Batch8_MTG, features = c("FCGR3A"),pt.size = 0)
```

CD56

```{r}
VlnPlot(Batch8_MTG, features = c("NCAM1"),pt.size = 0)
```
...
```{r}
VlnPlot(Batch8_MTG, features = c("KLRD1"),pt.size = 0)
```
NKG2A

```{r}
VlnPlot(Batch8_MTG, features = c("KLRC1"),pt.size = 0)
```
```{r}
VlnPlot(Batch8_MTG, features = c("CD244"),pt.size = 0)
```
...
```{r}
VlnPlot(Batch8_MTG, features = c("CD8A"),pt.size = 1)
```
...
```{r}
VlnPlot(Batch8_MTG, features = c("CD69"),pt.size = 0)
```



```{r}
VlnPlot(Batch8_MTG, features = c("TFRC"),pt.size = 0)
```



```{r}
VlnPlot(Batch8_MTG, features = c("IL2RA"),pt.size = 0)
```



```{r}
VlnPlot(Batch8_MTG, features = c("CTLA4"),pt.size = 0)
```



```{r}
VlnPlot(Batch8_MTG, features = c("FOXP3"),pt.size = 0)
```

```{r}
VlnPlot(Batch8_MTG, features = c("TRAC"),pt.size = 0)
```
...
```{r}
VlnPlot(Batch8_MTG, features = c("ENO2"),pt.size = 0)
```
Neurons: 0,1,2,3,5,6,7,8,9,10,13,15,16,18


```{R}
VlnPlot(Batch8_MTG, features = c("RBFOX3"),pt.size = 0)
```

Glutamatergic neuron markers

```{R}
VlnPlot(Batch8_MTG, features = c("SLC17A6"),pt.size = 1)
```
```{R}
VlnPlot(Batch8_MTG, features = c("SLC17A7"),pt.size = 0)
```
...
```{R}
VlnPlot(Batch8_MTG, features = c("SLC32A1"),pt.size = 1)
```
```{R}
VlnPlot(Batch8_MTG, features = c("GAD1"),pt.size = 0)
```


```{R}
VlnPlot(Batch8_MTG, features = c("GAD2"),pt.size = 0)
```
...
```{R}
VlnPlot(Batch8_MTG, features = c("AQP4"),pt.size = 0)
```


```{R}
VlnPlot(Batch8_MTG, features = c("GFAP"),pt.size = 0)
```
```{R}
VlnPlot(Batch8_MTG, features = c("ALDH1L1"),pt.size = 0)
```
ALDH1L1
...
```{R}
VlnPlot(Batch8_MTG, features = c("PLP1"),pt.size = 0)
```

```{R}
VlnPlot(Batch8_MTG, features = c("MBP"),pt.size = 0)
```


```{R}
VlnPlot(Batch8_MTG, features = c("MOG"),pt.size = 1)
```
...
```{R}
VlnPlot(Batch8_MTG, features = c("VCAN"),pt.size = 0)
```
```{R}
VlnPlot(Batch8_MTG, features = c("BCAN"),pt.size = 0)
```
...
```{R}
VlnPlot(Batch8_MTG, features = c("CX3CR1"),pt.size = 1)
```

```{R}
VlnPlot(Batch8_MTG, features = c("P2RY12"),pt.size = 1)
```
...
```{R}
VlnPlot(Batch8_MTG, features = c("FLT1"),pt.size = 0)
```


```{R}
VlnPlot(Batch8_MTG, features = c("CLDN5"),pt.size = 0)
```
...
data_barplot <- FetchData(Batch8_MTG, vars = c("ident",rownames(Batch8_MTG@assays$RNA@counts)), slot = "counts")
...
```{R}
#GeneSet <- rownames(GetAssayData(object = Batch8_MTG, assay.type = "RNA", slot = "counts"))
...
#  VlnPlot(Batch8_MTG, features = x,pt.size = 0)
...
```{R}
VlnPlot(Batch8_MTG, features = c("CLDN5","TH","P2RY12","GRM4","BCAN"),pt.size = 0,stack = TRUE, flip = TRUE)
```
...
```{r}
Batch8_MTG <- RenameIdents(Batch8_MTG, `0` = "GLU Neurons", `1` = "GLU Neurons", `2` = "GLU Neurons",
...
```{r}
DimPlot(Batch8_MTG, label = TRUE, repel = TRUE, pt.size = 0, label.size = 2)

UMAPclusters <- DimPlot(Batch8_MTG, label = TRUE, repel = TRUE, pt.size = 0, label.size = 2.5) + 
        theme(axis.text = element_text(size=8),
              axis.title = element_text(size = 12),
              legend.text = element_text(size = 8),
              title = element_text(size = 12),
...
```{r}
DimPlot(Batch8_MTG, label = TRUE, repel = TRUE, pt.size = 1, split.by = "case", label.size = 2,
        ncol = 1) + NoLegend()
```


```{r}
DimPlot(Batch8_MTG, label = TRUE, repel = TRUE, pt.size = 1,reduction = 'umap')
```
