...
layer_assignments <- read.delim("../Files/cell_type_to_layer_for_jacob_sc_all_05202022.csv", sep = ",")

melted_layer_assignments <- melt(layer_assignments)

Primary_Layer_Assignments <- group_by(melted_layer_assignments, X) %>% filter(value == max(value))
...
write.table(Combined_Predicted_CellType_Table, file = "../Files/AllSamples_Full_Combined_Predicted_CellType_Table.tsv", quote = FALSE, row.names = FALSE, sep = "\t")

Subset_Combined_Predicted_CellType_Table <- Combined_Predicted_CellType_Table[,c(-6,-8)]

write.table(Subset_Combined_Predicted_CellType_Table, file = "../Files/AllSamples_Combined_Predicted_CellType_Table.tsv", quote = FALSE, row.names = FALSE, sep = "\t")
