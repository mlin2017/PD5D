...
Run Harmony
```{r}
Batch7_MTG <- RunHarmony(Batch7_MTG, group.by.vars = c("sample_ID","case"), reduction = "glmpca", plot_convergence = TRUE, theta = c(1,1))
```
...
```{r}
Batch7_MTG <- FindNeighbors(Batch7_MTG, reduction = "harmony", dims = 1:17)
...
```{r}
Batch7_MTG <- RunUMAP(Batch7_MTG, reduction = "harmony", dims = 1:17)
```
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_GLMAPCA_UMAPclusters_scRNA_seq_Batch3_MTG.pdf", device = "pdf", width = 6, height = 4, units = "in")
```
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_Split_GLMPCA_UMAPclusters_scRNA_seq_Batch3_MTG.pdf", device = "pdf", width = 12, height = 4, units = "in")
```
