---
title: "Seurat Workflow and Cluster Assignment for Batch 11"
...
```{r}
Batch11.data <- Read10X(data.dir = "Files/cellranger_matrices/cellranger_aggr_matrices/filtered_feature_bc_matrix")
```
...
```{r}
Batch11_MTG <- CreateSeuratObject(counts = cbind(Batch11.data),
                            project = "Batch11_MTG",
                            min.cells = 3)
```
...
cellcodes <- as.data.frame(Batch11_MTG@assays$RNA@data@Dimnames[[2]])
...
Batch11_MTG@meta.data$sample_ID <- cellcodes$samples
...
Batch11_MTG@meta.data$case <- cellcodes$case
...
```{r}
table(Batch11_MTG$sample_ID)
```

```{R}
table(Batch11_MTG$case)
```
...
```{r}
Batch11_MTG[["percent.mt"]] <- PercentageFeatureSet(Batch11_MTG, pattern = "^MT-")

VlnPlot(Batch11_MTG, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
```
...
```{R}
library(stats)
nfeature_RNA <- Batch11_MTG@meta.data$nFeature_RNA
mean(nfeature_RNA)
MAD <- mad(nfeature_RNA, center = median(nfeature_RNA))
threeMAD <- (MAD*3)+median(Batch11_MTG@meta.data$nFeature_RNA)
...
```{r}
Batch11_MTG <- subset(Batch11_MTG, subset = nFeature_RNA > 200 & nFeature_RNA > threeMAD | percent.mt < 5)
```
...
```{r}
Batch11_MTG <- NormalizeData(Batch11_MTG, normalization.method = "LogNormalize", scale.factor = 10000)
```
...
```{R}
Batch11_MTG <- FindVariableFeatures(Batch11_MTG, selection.method = "vst", nfeatures = 2000)

# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(Batch11_MTG), 10)

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(Batch11_MTG)
...
```{r}
all.genes <- rownames(Batch11_MTG)
```

center and scale data
```{r}
Batch11_MTG <- ScaleData(Batch11_MTG, features = all.genes, verbose = FALSE)
```
...
```{r}
Batch11_MTG <- RunGLMPCA(Batch11_MTG, features=Batch11_MTG@assays$RNA@var.features, L = 30)
```

see contribution of genes to construct each of these principal components.
```{r}
VizDimLoadings(Batch11_MTG, dims = 1:2, reduction = "glmpca")
```


plot cells in the first two principal components colored by case: 
```{r}
DimPlot(object = Batch11_MTG, reduction = "glmpca", pt.size = .1, group.by = "case")
```

```{r}
glmpcaDimPlot_preHarmony <- DimPlot(object = Batch11_MTG, reduction = "glmpca", pt.size = .1, group.by = "case")
...
```{r}
VlnPlot(object = Batch11_MTG, features = "PC_1", group.by = "case",  pt.size = .1)
```

Run Harmony
```{r}
Batch11_MTG <- RunHarmony(Batch11_MTG, group.by.vars = c("case","sample_ID"), reduction = "glmpca", plot_convergence = TRUE, theta = c(1,1))
```


cells in harmony axis
```{r}
harmony_embeddings <- Embeddings(Batch11_MTG, 'harmony')
harmony_embeddings[1:5, 1:5]
```

cells in harmony axis
```{r}
DimPlot(object = Batch11_MTG, reduction = "harmony", pt.size = .1, group.by = "case")
```


```{R}

glmpcaDimPlot_postHarmony <- DimPlot(object = Batch11_MTG, reduction = "harmony", pt.size = .1, group.by = "case")
...
```{r}
VlnPlot(object = Batch11_MTG, features = "harmony_1", group.by = "case",  pt.size = .1)
```
...
```{R}
#Batch11_MTG <- JackStraw(Batch11_MTG, num.replicate = 100)
#Batch11_MTG <- ScoreJackStraw(Batch11_MTG, dims = 1:12)
#JackStrawPlot(Batch11_MTG, dims = 1:20)
...
ElbowPlot(Batch11_MTG, reduction = "glmpca")
...
ElbowPlot(Batch11_MTG, reduction = "harmony")
...
Batch11_MTG_ElbowPlot <- ElbowPlot(Batch11_MTG)

ggsave2("Figures/Batch11_MTG_ElbowPlot.pdf", Batch11_MTG_ElbowPlot, device = "pdf", width = 4, height = 4, units = "in")
```
...
```{r}
Batch11_MTG <- FindNeighbors(Batch11_MTG, reduction = "harmony", dims = 1:20)
Batch11_MTG <- FindClusters(Batch11_MTG, resolution = 0.5, algorithm = 4)
```

run Umap based on top 10 harmony axis: 

```{r}
Batch11_MTG <- RunUMAP(Batch11_MTG, reduction = "harmony", dims = 1:20)
```

run tsne based on top 10 harmony axis: 

```{r}
#Batch11_MTG <- RunTSNE(Batch11_MTG, reduction = "harmony", dims = 1:17)
```

plot umap: 
```{r}
DimPlot(Batch11_MTG, reduction = "umap", label = TRUE,pt.size = 0.01)
```

```{r}
DimPlot(Batch11_MTG, label = TRUE, repel = TRUE, pt.size = 0, label.size = 2)

UMAPclusters <- DimPlot(Batch11_MTG, label = TRUE, repel = TRUE, pt.size = 0, label.size = 2.5) + 
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_GLMAPCA_UMAPclusters_scRNA_seq_Batch11_MTG.pdf", device = "pdf", width = 6, height = 4, units = "in")
```


```{R}
UMAPclusters <- DimPlot(Batch11_MTG, label = TRUE, repel = TRUE, pt.size = 0, label.size = 2.5, split.by = "case") + 
        theme(axis.text = element_text(size=8),
              axis.title = element_text(size = 12),
              legend.text = element_text(size = 8),
              title = element_text(size = 12),
...
ggsave(UMAPclusters, filename = "Figures/Unassigned_Split_GLMPCA_UMAPclusters_scRNA_seq_Batch11_MTG.pdf", device = "pdf", width = 12, height = 4, units = "in")
```

Find markers for every cluster compared to all remaining cells, report only the positive ones
```{r}
Batch11_MTG.markers <- FindAllMarkers(Batch11_MTG, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
```
...
```{r}
unique(Batch11_MTG.markers$cluster)
```

Top marker genes for each clusters: 
```{r}
Batch11_MTG.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
write.table(Batch11_MTG.markers, file = "Files/AllMarkers.txt", col.names = TRUE, sep = "\t", quote = FALSE)
top10Markers <- Batch11_MTG.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
...
DoHeatmap(Batch11_MTG, features = features, size = 2, draw.lines = FALSE, angle = 45,
...
```{r}
FeaturePlot(Batch11_MTG, features = c("ENO2", "RBFOX3","SLC17A6", "SLC17A7","AQP4", "GFAP"))
```

```{r}
saveRDS(Batch11_MTG,"Files/Batch11_MTG_Unassigned.rds")
```
...
```{R}
Batch11_MTG=readRDS("Files/Batch11_MTG_Unassigned.rds")
```
...
```{r}
VlnPlot(Batch11_MTG, features = c("CD96"),pt.size = 0)
```
NK Cell Markers

```{r}
VlnPlot(Batch11_MTG, features = c("IL7R"),pt.size = 0)
```
...
```{r}
VlnPlot(Batch11_MTG, features = c("FCGR3A"),pt.size = 0)
```

CD56

```{r}
VlnPlot(Batch11_MTG, features = c("NCAM1"),pt.size = 0)
```
...
```{r}
VlnPlot(Batch11_MTG, features = c("KLRD1"),pt.size = 0)
```
NKG2A

```{r}
VlnPlot(Batch11_MTG, features = c("KLRC1"),pt.size = 0)
```
```{r}
VlnPlot(Batch11_MTG, features = c("CD244"),pt.size = 0)
```
...
```{r}
VlnPlot(Batch11_MTG, features = c("CD8A"),pt.size = 1)
```
...
```{r}
VlnPlot(Batch11_MTG, features = c("CD69"),pt.size = 0)
```



```{r}
VlnPlot(Batch11_MTG, features = c("TFRC"),pt.size = 0)
```



```{r}
VlnPlot(Batch11_MTG, features = c("IL2RA"),pt.size = 0)
```



```{r}
VlnPlot(Batch11_MTG, features = c("CTLA4"),pt.size = 0)
```



```{r}
VlnPlot(Batch11_MTG, features = c("FOXP3"),pt.size = 0)
```

```{r}
VlnPlot(Batch11_MTG, features = c("TRAC"),pt.size = 0)
```
...
```{r}
VlnPlot(Batch11_MTG, features = c("ENO2"),pt.size = 0)
```
Neurons: 0,1,2,3,5,6,7,8,9,10,13,15,16,18


```{R}
VlnPlot(Batch11_MTG, features = c("RBFOX3"),pt.size = 0)
```

Glutamatergic neuron markers

```{R}
VlnPlot(Batch11_MTG, features = c("SLC17A6"),pt.size = 1)
```
```{R}
VlnPlot(Batch11_MTG, features = c("SLC17A7"),pt.size = 0)
```
...
```{R}
VlnPlot(Batch11_MTG, features = c("SLC32A1"),pt.size = 1)
```
```{R}
VlnPlot(Batch11_MTG, features = c("GAD1"),pt.size = 0)
```


```{R}
VlnPlot(Batch11_MTG, features = c("GAD2"),pt.size = 0)
```
...
```{R}
VlnPlot(Batch11_MTG, features = c("AQP4"),pt.size = 0)
```


```{R}
VlnPlot(Batch11_MTG, features = c("GFAP"),pt.size = 0)
```
```{R}
VlnPlot(Batch11_MTG, features = c("ALDH1L1"),pt.size = 0)
```
ALDH1L1
...
```{R}
VlnPlot(Batch11_MTG, features = c("PLP1"),pt.size = 0)
```

```{R}
VlnPlot(Batch11_MTG, features = c("MBP"),pt.size = 0)
```


```{R}
VlnPlot(Batch11_MTG, features = c("MOG"),pt.size = 1)
```
...
```{R}
VlnPlot(Batch11_MTG, features = c("VCAN"),pt.size = 0)
```
```{R}
VlnPlot(Batch11_MTG, features = c("BCAN"),pt.size = 0)
```
...
```{R}
VlnPlot(Batch11_MTG, features = c("CX3CR1"),pt.size = 1)
```

```{R}
VlnPlot(Batch11_MTG, features = c("P2RY12"),pt.size = 1)
```
...
```{R}
VlnPlot(Batch11_MTG, features = c("FLT1"),pt.size = 0)
```


```{R}
VlnPlot(Batch11_MTG, features = c("CLDN5"),pt.size = 0)
```
...
data_barplot <- FetchData(Batch11_MTG, vars = c("ident",rownames(Batch11_MTG@assays$RNA@counts)), slot = "counts")
...
```{R}
#GeneSet <- rownames(GetAssayData(object = Batch11_MTG, assay.type = "RNA", slot = "counts"))
...
#  VlnPlot(Batch11_MTG, features = x,pt.size = 0)
...
```{R}
VlnPlot(Batch11_MTG, features = c("CLDN5","TH","P2RY12","GRM4","BCAN"),pt.size = 0,stack = TRUE, flip = TRUE)
```
...
```{r}
Batch11_MTG <- RenameIdents(Batch11_MTG, `0` = "GLU Neurons", `1` = "GLU Neurons", `2` = "GLU Neurons",
...
```{r}
DimPlot(Batch11_MTG, label = TRUE, repel = TRUE, pt.size = 1, split.by = "case", label.size = 2,
        ncol = 1) + NoLegend()
```


```{r}
DimPlot(Batch11_MTG, label = TRUE, repel = TRUE, pt.size = 1,reduction = 'umap')
```
